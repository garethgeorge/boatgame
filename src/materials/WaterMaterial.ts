import { Scene, ShaderMaterial, Vector3, Vector2, Color3, Effect } from '@babylonjs/core';

export class WaterMaterial extends ShaderMaterial {
  constructor(name: string, scene: Scene) {
    super(name, scene, {
      vertexSource: WaterShader.vertexShader,
      fragmentSource: WaterShader.fragmentShader,
    }, {
    ...
    // Default values
    this.setFloat("uTime", 0);
    this.setColor3("uColor", new Color3(0.3, 0.65, 1.0)); // 0x4da6ff
    this.setVector2("uFlowDirection", new Vector2(0, 1));
    this.setVector3("uSunPosition", new Vector3(50, 100, 50));
    this.setVector3("uBoatPosition", new Vector3(0, 0, 0));
    this.setVector2("uBoatVelocity", new Vector2(0, 0));
    this.setVector2("uBoatDirection", new Vector2(0, -1));
    this.setFloat("uSwampFactor", 0.0);
    this.setColor3("uSwampColor", new Color3(0.18, 0.31, 0.18)); // 0x2f4f2f

    // History array initialization
    const history: number[] = [];
    for (let i = 0; i < 8 * 3; i++) history.push(0);
    this.setArray3("uBoatHistory", history);
...
const WaterShader = {
  ...
  fragmentShader: `
    ...
    if (uSwampFactor > 0.01) {
      ...
      // Reduce flow line intensity in swamp
      float flowIntensity = 1.0 - uSwampFactor * 0.8; 
