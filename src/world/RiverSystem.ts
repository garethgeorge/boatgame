...
export class RiverSystem {
    ...
    private constructor(biomeManager: BiomeManager) {
        this.noise = new SimplexNoise(100);
        this.biomeManager = biomeManager;
        this.terrainGeometry = new TerrainGeometry(this);
    }

    public static createInstance(biomeManager: BiomeManager): RiverSystem {
        if (RiverSystem.instance) {
            throw new Error('RiverSystem instance already exists');
        }
        RiverSystem.instance = new RiverSystem(biomeManager);
        return RiverSystem.instance;
    }

    public static getInstance(): RiverSystem {
        if (!RiverSystem.instance) {
            throw new Error('RiverSystem.createInstance() must be called before getInstance()');
        }
        return RiverSystem.instance;
    }
