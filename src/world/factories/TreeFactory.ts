...
class WillowTreeConfig implements TreeConfig {
    ...
    addLeaves(leafGeos: THREE.BufferGeometry[], matrix: THREE.Matrix4, length: number, wetness: number): void {
        ...
        for (let i = 0; i < strandCount; i++) {
            ...
            const strandGeo = new THREE.BoxGeometry(sWidth, sLen, 0.05, 1, 16, 1);
    ...
    getLeafColor(isSnowy: boolean): THREE.Color {
        return isSnowy ? new THREE.Color(0xFFFFFF) : new THREE.Color(0x7c9b4e);
    }
}
...
export class TreeFactory implements DecorationFactory {
    ...
    private static readonly leafMaterial = new THREE.MeshToonMaterial({ color: 0xffffff, name: 'Tree - Leaf Material', side: THREE.DoubleSide }); // White
