...
type DecorationEntity = Decoration | Pier | Collectible | Obstacle;
...
export class RiverGenerator {
    ...
    update(playerPosition: THREE.Vector3, elapsedTime: number) {
    ...
    createChunkMesh(zStart: number, zEnd: number, segments: number): THREE.Mesh {
        const vertices = [];
        const indices = [];
        const colors = [];
    ...
    createWaterMesh(zStart: number, zEnd: number, segments: number): THREE.Mesh {
        const vertices = [];
        const indices = [];
    ...
    addDecorations(zStart: number, zEnd: number): DecorationEntity[] {
    ...
    getRiverTangent(z: number): THREE.Vector3 {
        return this.riverPath.getTangentAt(z);
    }

    getRiverCenter(z: number): THREE.Vector3 {
