...
export abstract class ShoreAnimalDecorator extends BaseDecorator {
    protected async decorateShoreAnimal(
        ...
        for (let i = 0; i < count; i++) {
            ...
            const normal = context.geometry.calculateNormal(placement.localX, placement.worldZ);
    ...
    private placeShoreAnimal(
        ...
        const terrainNormal = context.geometry.calculateNormal(placement.localX, placement.worldZ);
        ...
        if (animalData.animations.length > 0) {
            ...
            action.play();
            // We need to add mixer to chunk's mixers. 
            // But context.chunk is available.
            // We need to expose addMixer or access mixers public/internal.
            // TerrainChunk has private mixers.
            // I should add a public method to TerrainChunk to add mixer.
            // For now, I'll cast to any or add method later.
            // Let's assume I'll add `addMixer` to TerrainChunk.
            (context.chunk as any).addMixer(mixer);
        }
