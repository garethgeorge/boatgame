...
export class Hippo extends Entity {
    ...
    static HippoBehavior = class implements EntityBehavior {
        ...
        update(dt: number) {
            ...
            if (this.state === 'PREPARING') {
                this.chargeTimer -= dt;
                if (this.hippo.meshes.length > 0) {
                    // Shake effect only, no tilt
                    const mesh = this.hippo.meshes[0];
        ...
        private updateAI() {
            ...
            if (!targetBody || this.hippo.physicsBodies.length === 0) return;

            const physicsBody = this.hippo.physicsBodies[0];
    ...
    constructor(x: number, y: number, physicsEngine: PhysicsEngine, angle: number = 0) {
        ...
        this.behavior = new Hippo.HippoBehavior(this);
    }
